        ...
        $(function() {
            ...
            $(document).on("click",'.filtersave',function() {
                ...
                $(".devicedetailtr").each(function() {
                    ...
                    details.attentname= d.eq(0).text();
                    details.attentspecial= d.eq(1).text();
                    details.attentnum= d.eq(2).text();
                ...
                var attentrequire=$(".attentrequireval").val().trim();
                ...
                if(person!=''&&abstract!=''&&reason!=''&&receive!='0') {//第一次校验成功了
                    ...
                    //发送ajax请求后台服务器
                    $.ajax({
                        url:"${pageContext.request.contextPath}/applyattentplan.do",
                        type:"post",
                        async:false,
                        data:{"details":JSON.stringify(saveDataAry),"user.uid":uid,"attenttitle":othertitle,"applyperson":person,"applyabstract":abstract,"receive":receive,"applytime":startime,"buyreson":reason,"flowid":flowid,"attentrequire":attentrequire},
                        dataType:"json",
                        success:function(data) {
                            ...
                            if(data=='success') {
                                ...
                                alert("流程提交失败!");
                                window.location="${pageContext.request.contextPath}/climpattentpage.do";
...
<div class="top">
    ...
    <span class="title">其他计划申请页面</span>
...
<div class="mai">
    ...
    <div hidden="hidden" class="flowid">5</div>
    ...
    <div class="appdetail">申请原因<span style="color: red">*</span></div>

    <div class="receive">选择采购部门<span style="color: red">*</span></div>
    ...
    <div class="buydetail">填写计划明细</div>

    <table class="bydetailtb" style="margin-top: 4%">

        <tr>

            <td width="5%" style="border-left: 1px solid #dfe1e5">名称</td>
            <td width="30%"><input style="font-size: 15px" class="productname" type="text" /></td>
            <td width="5%">规格</td>
            <td width="30%"><input style="font-size: 15px" class="productspecial" type="text" /></td>
            <td width="5%">数量</td>
            <td width="10%"><input style="font-size: 15px" class="productnum" type="text" /></td>
            <td width="15%" style="border-right: 1px solid #dfe1e5"><input class="cancel" type="button" value="取消" /> <input class="add" type="button" value="添加" /></td>

        </tr>

    </table>

    <div class="attentrequire">要求</div>

    <input class="attentrequireval" type="text" />

    <div class="devicelist">计划明细单</div>

    <table class="devicelisttb">

        <tr style="font-weight: bold">

            <td width="50%" style="border-left: 1px solid #dfe1e5">名称</td>
            <td width="30%">规格</td>
            <td width="10%">数量</td>
