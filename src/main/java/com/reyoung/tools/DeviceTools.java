...
public class DeviceTools {
    ...
    //负责部门为本部门的签字审批
    public static String makereport(DevicePlan devicePlan,Flowinfos flowinfos, java.util.List<Approve> approves,HttpServletRequest request) {
        ...
        try{
            ...
            Paragraph elements = new Paragraph(devicePlan.getDevicetitle(), setChineseFont(request));
            ...
            datatable.addCell(createrepairecontext(devicePlan.getBuyreson(), 5, 1,request));//将前后空格去掉后在前面加3个tab
            ...
            if (res==0) {//没有购买的详情的信息
                ...
                datatable.addCell(createrepairerequiresdetail("品牌", 2, 1,request));
                datatable.addCell(createrepairerequiresdetail("数量", 1, 1,request));

                for (DeviceDetail detail:devicePlan.getDeviceDetails()) {

                    datatable.addCell(createrepairerequiresdetail(detail.getDevicename(),2,1,request));
                    datatable.addCell(createrepairerequiresdetail(detail.getDevicebank(),2,1,request));
                    datatable.addCell(createrepairerequiresdetail(detail.getDevicenum(),1,1,request));

                }

                //获取采购设备的要求
                datatable.addCell(createrepairerequiresother(devicePlan.getDevicerequire(), 6, 1,request));
    ...
    //负责部门是其他单位的签字审批
    public static String makereport1(DevicePlan devicePlan,Flowinfos flowinfos, java.util.List<Approve> approves,HttpServletRequest request) {
        ...
        try{
            ...
            String s = request.getSession().getServletContext().getRealPath(File.separator+"makereports"+File.separator);
